...
class MessageConstructor:
    ...
    @staticmethod
    def construct_completion_inputs(chat_history, new_content = None, multimodal_contents: List = []) -> List: 
        ...
        if not new_content and multimodal_contents:
            completion_inputs.append({"role": "user", "content": [" "] + multimodal_contents})

        if new_content:
            completion_inputs.append({"role": "user", "content": [new_content] + multimodal_contents})
