...
class GraphExecutor:
    ...
    def execute(self):
        ...
        for layer_index, layer in enumerate(self.execution_plan, 1):
            logger.info(
                f"Processing execution layer {layer_index}/{len(self.execution_plan)}: {layer}"
            )
            ...
            with logger.contextualize(node=node_name):
                ...
                logger.success(
                    f"Node execution complete. Output: {current_node_executed_data}"
                )
                ...
                for successor_index, successor_name in enumerate(successors, 1):
                    with logger.contextualize(successor=successor_name):
                        logger.debug(
                            f"Processing successor {successor_index}/{len(successors)}"
                        )

                        edge_data = self.graph.get_edge_data(node_name, successor_name)
                        source_handle = edge_data.get("source_handle").split("-index")[
                            0
                        ]
                        target_handle = edge_data.get("target_handle").split("-index")[
                            0
                        ]
                        ...
                        # Log before update
                        logger.debug(
                            f"Current successor input values: {successor_node_data.input_values}"
                        )

                        # Update the values
                        successor_input_values = successor_node_data.input_values

                        successor_input_values[target_handle] = (
                            current_node_executed_data.output_values[source_handle]
                        )

                        # Log after update
                        logger.debug(
                            f"Updated successor input values: {successor_input_values}"
                        )
