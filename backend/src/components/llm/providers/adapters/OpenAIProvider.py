...
class OpenAIProvider(LLMProviderBase):
    """
    OpenAI provider adapter for LLM integration.

    This adapter provides a clean interface to interact with OpenAI models
    ...
    def get_client(self) -> instructor.Instructor:
        """
        Get the instructor client for OpenAI.

        Returns:
            Instructor client configured for OpenAI
    ...
    def chat_completion(
        ...
        """
        Perform a chat completion with the OpenAI model.
    ...
    def role_fix(self, chat_message: ChatMessage) -> ChatMessage:
        """
        Fix the role of a chat message to match OpenAI's expected roles.
