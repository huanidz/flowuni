          ...
          {flows.map(flow => (
            <React.Fragment key={flow.flow_id}>
              <TableRow>
                ...
                <TableCell>
                  <Button onClick={() => handleFlowClick(flow)} variant="ghost" size="sm">
                    Run
                  </Button>
                  <Button
                ...
                <TableCell colSpan={3}>
                  <div>
                    {expandedRows.has(flow.flow_id) && (
                      <div>
                        <div>
                          <div>ID: {flow.flow_id}</div>
                          <div>Ngày tạo: {new Date(flow.created_at as string).toLocaleDateString('vi-VN')}</div>
                          <div>Số lượng nút: {flow.node_count !== undefined ? flow.node_count : 'Không có số lượng nút'}</div>
                          <div>Trạng thái: {flow.is_active ? 'Đang hoạt động' : 'Tạm dừng'}</div>
                        </div>
                        <div>
                          <div>Mô tả: {flow.description}</div>
                        </div>
                        <div>
                          <div>Tình trạng: {flow.is_active ? 'Dòng chảy đang hoạt động' : 'Dòng chảy tạm dừng'}</div>
                        </div>
                      </div>
                    )}
                  </div>
